...
$(".side-menu").on('click', 'li a', function(e) {
	...
	e.preventDefault();
	//yr
	openTab($this.find("span").text(),$this.attr("href"));
});
...
function openTab(title, url){
	...
	if (!isHas) {
		var tab = "<a href='javascript:;' class='content-tab active' data-id='"+h+"' id='tab_box_"+h+"'>"+ label +" <i class='icon-font' id='tab_x_"+h+"'>&#xe617;</i></a>";
		$(".content-tab").removeClass("active");
		$(".tab-nav-content").append(tab);
		var iframeid="iframe" + m;
		/*
		...
		var iframe = "<iframe class='body-iframe' id='"+ iframeid +"' name='"+ iframeid 
		+"' width='100%' height='99%' src='"+ h +"' frameborder='0' data-tabname='"
		+label+"' data-id='"+ h +"' data-iframeid='"+ iframeid +"' seamless></iframe>";
...
//刷新列表
function refreshParentList(tabName,closeCurrentTab){
	$(".body-iframe").each(function() {
		if ($(this).data("tabname") ==tabName){
			var cur_tab_id=$(this).data("id");
			var iframeid = $(this).data("iframeid");
			document.getElementById(iframeid).contentWindow.searchTable();
			//刷新列表同时关闭当前选项卡
			if(closeCurrentTab){
				closeTab();
				document.getElementById("tab_box_"+cur_tab_id).click();
			}
		}
	});
}
...
function closeTab(){
	...
	$(".content-tab").each(function() {
			if ($(this).hasClass("active")) {
				cur_tab_id = $(this).data("id");
			}
	});
	document.getElementById("tab_x_"+cur_tab_id).click();
}
/*选项卡切换方法*/
function navChange() {
...
function getThisFrameId(thisframe) {
    var frames = document.getElementsByTagName("iframe");

    for (i = 0; i < frames.length; i++) { //遍历，匹配时弹出id
        if (frames[i].contentWindow == thisframe) {
        	var obj=frames[i].id;
            return obj;
        }
    }
